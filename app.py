# app.py — Sentinel v2.4 (Clean Demo Build)
# -----------------------------------------
# Fixes: header typing/fade, spacing, Ask-Agent trigger reliability
# Includes: detonator indicator, handoff flash, sonar ping, signal flash
...
/* Header (fixed typing then fade) */
.header-container{
  text-align:center; margin-top:6px;
  animation: headerFadeOut 1s ease 7.2s forwards;
}
.typewriter-title{
  display:inline-block; overflow:hidden; white-space:nowrap;
  border-right:.15em solid var(--accent);
  letter-spacing:.08em; color:var(--accent);
  font-weight:700; font-size:46px; width:0;
  animation:typingTitle 2.6s steps(30,end), blink-caret .75s step-end 3;
...
.typewriter-tagline{
  ...
  font-size:14px; margin-top:6px; width:0;
  animation:typingTag 3.4s steps(40,end) 2.7s, blink-caret .75s step-end 3 2.7s;
  animation-fill-mode:forwards;
}
...
if thread:
    for item in thread[-15:]:
        ...
        st.markdown(f"""<div class="chat-bubble{css_extra}"><b>{item['agent'].upper()}</b><br>{item['response'].replace(chr(10),'<br>')}<div class="meta">⏱ {item['time']}</div></div>""",unsafe_allow_html=True)
else:
...
def run_agent(agent_key,user_q):
    print(f"DEBUG: run_agent triggered for {agent_key} with {len(user_q)} chars")
    ...
    try:
        ...
        if isinstance(parsed,dict):
            pretty=""
            for k in ["summary","insights","next_steps"]:
                if parsed.get(k): pretty+=f"**{k.capitalize()}:** {parsed[k]}\n\n"
            output=pretty or output
    except: pass
    st.session_state["threads"][agent_key].append({"agent":agent_key,"query":user_q,"response":output,"time":datetime.now().strftime("%H:%M:%S"),"error":is_error})
    st.session_state["is_running"]=False; st.rerun()

# ---------- Footer ----------
st.markdown("<div class='static-footer'><div class='footer-inner'><div class='footer-left'></div><div class='footer-right'></div></div></div>",unsafe_allow_html=True)
