# app.py — Sentinel v3.1.6 (Analyst Console Layout)
# -----------------------------------------------------------
# Chat window ABOVE; prompt + buttons BELOW (Slack-style).
# Clean alignment, no sidebar, cinematic header, static theme.
# Reliable subprocess agent calls with retry + safe caps + handoff summary.
...
/* Center wrapper */
.content-inner{max-width:1100px;margin:0 auto;padding:0 1.2rem 4rem;}
...
.chat-wrap{
  ...
  border-radius:12px;padding:16px 18px;height:60vh;overflow-y:auto;margin:10px 0 16px;
}
.chat-bubble{
...
/* Input block (below chat) */
.input-block{margin-top:6px;}
.stTextArea textarea{
...
/* Status chip & overlay */
.status-chip{position:fixed;top:10px;right:14px;background:#1f242b;border:1px solid var(--border);
  color:#fff;padding:6px 10px;border-radius:999px;font-size:12px;box-shadow:0 2px 10px rgba(0,0,0,.25);z-index:10000;}
...
.loading-overlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center;color:#fff;font-size:18px;font-weight:600;z-index:9998;backdrop-filter:blur(3px);}
</style>
...
with colR:
    st.markdown("<div class='version-label'>v3.1.6</div>", unsafe_allow_html=True)

# ---------- Status + overlay ----------
def _status_ui():
    if st.session_state.get("is_running") and st.session_state.get("active_agent"):
        st.markdown(
            f"<div class='status-chip'><span class='status-dot'></span>🛰 Accessing <b>{st.session_state['active_agent'].upper()}</b>…</div>",
            unsafe_allow_html=True
        )
...
st.session_state["prompt"] = st.text_area(
    ...
    key="prompt_box", placeholder=f"Ask {agent.capitalize()}…", height=92
)
...
# ---------- Attach Context & Prompting Guide ----------
...
# ---------- UX niceties: focus + autoscroll ----------
st.markdown("""
<script>
setTimeout(()=>{const ta=window.parent.document.querySelector('textarea'); if(ta){ta.focus();}},350);
